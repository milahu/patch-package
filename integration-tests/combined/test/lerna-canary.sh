...
expect_ok "making patch" npx patch-package @parcel/codeframe

expect_ok "the patch looks like this" cat patches/@parcel+codeframe+2.0.0-nightly.137.patch

echo "reinstall node_modules"
rm -rf node_modules
yarn

expect_error "patch-package didn't run" grep yarn node_modules/@parcel/codeframe/src/codeframe.js

expect_ok "the patch applies" npx patch-package
